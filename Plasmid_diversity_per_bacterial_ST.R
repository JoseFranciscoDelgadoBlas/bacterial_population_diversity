#####################################################
## Plasmid Diversity Calculations per Bacterial ST ##
#####################################################
...
# Presence/absence plasmid incompatibility group data from ARIBA pipeline using PlasmidFinder database
plasmidMatrix <- fread("~/plasmid_content.csv")

# File with samples Ids and origin tag for each sample (WWTP/river)
mapping <-  fread("~/Mapping.csv")

## Calculate jaccard similarity matrix
dist.jac <- vegdist(plasmidMatrix, method="jaccard", header=FALSE, binary=TRUE, diag=FALSE, upper=FALSE, lower=TRUE)
...
# Divide the values from similaraty matrix by the number of different STs of the same bacterial species in this origin (river)
dist_rivers <- dist.jac.matrix[river_ids, river_ids] / 6
dist_rivers <- upperTriangle(dist_rivers, diag=FALSE)

# Divide the values from similaraty matrix by the number of different STs of the same bacterial species in this origin (WWTP)
dist_wwtp <- dist.jac.matrix[wwtp_ids, wwtp_ids] / 4
